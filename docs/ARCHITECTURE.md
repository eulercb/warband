...
| `content/` | Data-driven game content: player `classes`, `monsters` (33 bosses, tiers, run assembly, endless modifiers), boss `affixes` (Vampiric/Frenzied/… + the seeded roll policy), generic run `upgrades`, per-class `charUpgrades` (incl. hybrids), player-count `scaling`, and the adaptive match `balance` engine (kill-pace + band-power difficulty adjustment). |

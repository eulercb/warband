...
describe('classifyForBot — from components', () => {
  ...
  it('reads the recombined payload for its role', () => {
    expect(role({ delivery: { kind: 'heal' }, effects: [{ kind: 'heal', amount: 40 }] })).toBe(
      'heal',
    );
    expect(
      role({
    ...
    expect(
      role({
        delivery: { kind: 'groundZone' },
        effects: [
          { kind: 'zone', zone: { zoneKind: 'sanctuary', radius: 100, duration: 4, tickHeal: 8 } },
        ],
      }),
    ).toBe('allyBuff');
    expect(
    ...
    expect(
      role({
        delivery: { kind: 'meleeCone', range: 80 },
...
describe('decideAbilitiesForged — casting policy', () => {
  ...
  it('pops a self-defense buff when low on HP', () => {
    ...
    bot.abilities!.a1 = synth({
      delivery: { kind: 'selfBuff' },
      effects: [{ kind: 'buff', target: 'self', defMult: 0.5, duration: 4 }],
    });
  ...
  it('fires control and ally-buff abilities while engaged', () => {
    ...
    bot.abilities!.a2 = synth({
      ...
      effects: [{ kind: 'buff', target: 'allies', dmgMult: 1.2, duration: 5 }],
    });
    const cmd = computeBotInput(w, bot, w.tick);
    expect(cmd.buttons.a1).toBe(true);
  ...
  it('casts subclass slots, respects cooldowns/edges, and skips empty slots', () => {
    ...
    w.boss!.pos = { x: 560, y: 500 };
    const shot = () =>
      synth({ delivery: { kind: 'projectile' }, effects: [{ kind: 'damage', amount: 20 }] });

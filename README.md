...
- **Walkable main menu** — the app opens straight into a playable playground: your hero standing in the arena with a respawning **training dummy** and four **totems**. Walk onto a totem to host a game, join one, change your hero or open the controls; or just swing at the dummy to learn your kit before ever touching a menu. A row of **class effigies** lines the camp — walk onto one and your hero transforms into that class on the spot, no menu required. (The classic buttons + picker stay in the corner panel.)
- **12 classes** — the full DnD base-class lineup: Knight, Ranger, Mage, Cleric, **Barbarian** (rage bruiser), **Rogue** (burst + evasion), **Paladin** (off-tank/support hybrid), **Druid** (nature control), plus **Bard** (support/control), **Monk** (melee skirmisher), **Sorcerer** (burst DPS + mobility) and **Warlock** (ranged, life-draining attrition), each with a distinct role and ability kit.
- **Seeded runs, Run of the Day & Hardcore** — a gauntlet's whole session (its boss set, each boss's affixes, terrain _and_ the between-boss reward offers) derives from one **master seed**, so a run is reproducible. Host with a fresh **random** seed, the shared **Run of the Day** (UTC date — everyone gets the same run), or a **custom seed** you type in to replay an exact boss set + reward sequence. A gauntlet never lets you hand-pick the opener (it's a seeded random draw), and each **Endless** lap re-rolls a **fresh boss set**. Opt into **Hardcore** for a deadlier run: mid-fight corruption comes thick and _accelerates_ the longer a boss survives, revives are limited per fight, and a wipe ends the run — no free retry.
- **Procedurally rolled kits, monsters & boons — every run, same identities** — hosting a session re-rolls the _details_ of the whole content set from the run's master seed while keeping every identity intact. Each class keeps its role and ability shapes, but this run’s numbers — damage, cooldowns, reach, radii, cast/stun/slow/buff timings — and even the skill names roll fresh from hand-authored, mechanic-true banks (one run’s Fireball is a snappy little **"Emberburst"**, the next a slow, huge **"Pyroclasm"**); monsters keep their AI and telegraph shapes but roll stats, ability numbers and an epithet (**"Broodmother, the Ravenous"**); the generic boons roll their magnitudes with names and descriptions regenerated to match. Rolls are **budget-linked** — a harder-hitting skill cycles slower, a bulkier boss hits softer — so a lucky seed changes a run’s _feel_, not its fairness, and telegraph wind-ups never dip below readability floors. Everything derives from the shared master seed, so the whole band sees identical content, the lobby’s class cards already preview the run’s rolled kit, a **custom seed** replays the exact same arsenal, and the **Run of the Day** hands everyone the same rolled world. The menu playground keeps the canonical kits as a stable training baseline.
- **Grand improvements & bot progression** — each class has rare, transformative **grand improvements** (unique capstones you can hold only once) that surface as a prized pick. AI band members **progress their power** between bosses alongside the humans: after every boss each bot takes a generic run boon **and** advances its kit — drawing from its class pool _and_ the class-agnostic **hybrid** grafts/combat-styles at the same rate a human's offer is swapped for one — with personality shaping the choices (a Reckless bot hoards damage, a Timid one stacks health).
- **Mobile-friendly touch controls** — on a phone/tablet a twin-stick overlay appears: a left stick to move, a right stick to aim, on-screen ability buttons and Revive. The same overlay drives the fight, the reward room and the menu playground.
- **Accessibility** — a **screen-shake toggle** (persisted) for players who find the camera jolt uncomfortable.
...
- **5-boss runs + Endless, now with MULTI-boss packs** — a run climbs through five encounters of rising difficulty with an upgrade phase between each; past the opener, any encounter can roll into a shared-arena pack — a **twin**, and now sometimes a **triplet or quad** — of different bosses (each weakened further the more of them share the floor: chaos, not N× damage), splitting their attention across your top threats. Clear the run and the Victory screen offers **Endless**: carry every upgrade and score into escalating cycles where bosses gain an elemental **type** (Frost Dragon, Dark Troll…), packs get more common, and everything grows deadlier each lap.
- **Weightier combat** — attack cooldowns are longer and boss HP is scaled to match, so each blow lands less often but hits harder and fights hold their length; a regenerating boss can no longer out-heal a committed party (its regen is suppressed while you keep up the pressure).
- **Boss affixes — the same boss, a wildly different fight** — affixes and mid-fight corruption are now **standard on every fight** (the opener carries a single light affix and corruption's grace delay keeps the first fight readable, ramping from there). Each boss rolls one or more **affixes** that reshape how you fight it, Diablo/PoE-map-mod style: **Vampiric** (heals off the damage it deals), **Frenzied** (attacks faster the lower its health falls), **Splitting** (calls extra add waves on top of its kit), **Volatile** (its telegraphed impacts leave boiling pools), **Molten** (trails burning ground as it moves), **Warding** (periodic damage-absorbing shields), **Accelerating** (speeds up the longer the fight drags), **Teleporting** (blinks away the instant you crowd it — even bosses that normally can't), **Overcharged** (bigger telegraphs _and_ bigger hits) and **Barbed** (a thorn burst that answers your heaviest blows). They **stack** — a _Vampiric Frenzied_ Orc plays nothing like a _Volatile Molten_ one — surfacing as coloured name chips on the boss bar and an "elite" **aura corona** around the boss. A built-in **fairness governor** keeps two bosses' (or a boss + a corruption strike's) telegraphs from composing into an undodgeable wall unless the boss is deliberately enraged.
- **Mid-fight corruption events** — seeded beats fire _during_ a fight (Hades-encounter / RoR2-void-seed style) to keep even a boss you know unpredictable: an **Enrage Surge** (a brief band-wide roar of extra damage + speed), a **Telegraph Rain** of delayed strikes near the party, erupting **hazard Vents** that leave lingering pools (procedural terrain, mid-fight), an **Add Swarm**, and a **Collapsing Arena** ring that herds the band to the centre — plus, now and then, a benevolent **Healing Rift** that rewards good positioning. Each is announced with a center-screen banner, ramps up through Endless cycles, and — like affixes — is only enabled for gauntlet runs past the opener and every Endless cycle, so a single featured fight stays a clean duel.
...
- **Multiclass swapping** — clear a run and a special reward can grant your hero a whole extra **class**, and you carry every owned class into the next fight — each with its own kit and its own cooldowns (they tick down while you wield another). **Tap** the swap button to cycle to the next class, or **hold** it to open a **class radial** and release onto the exact class you want; works on keyboard, gamepad and touch (drag the on-screen Swap button toward a class). A short gate stops free-cast swap spam.
- **Walkable muster hall** — the lobby is a hall you stand in: step onto the **muster rune** to ready up (step off / step on again to stand down), and — if you host — onto the **war-horn** to sound the advance once the whole band is ready (it stays sealed until then). The room code, share link, live roster and bot/host controls keep their panel, docked to the side, so nothing about coordinating a game is lost.
- **Walkable war room** — picking your fight isn't a grid either: hosting drops you into a war room where the bosses stand risen as **effigies** behind **Easy / Medium / Hard gates**. Walk up to the one you want to set your run's opener (its silhouette and lore surface as you approach), step into the **Single-fight** or **Gauntlet** portal to choose the mode, then walk into the **Raise Your Banner** portal (a deliberate channel) to host. The classic host-setup form stays one click away as the List view for pointer / keyboard / screen-reader players.
...
- **Bard** — support/control. Vicious Mockery, an Inspiration buff, Healing Word, and Dissonant Whispers.
- **Monk** — melee skirmisher. Flurry of Blows, a Stunning Strike, Step of the Wind, and Quivering Palm.
- **Sorcerer** — burst DPS + mobility. Twin Chaos Bolt, a charged Meteor, Mirror Image, and an Arcane Leap.
- **Warlock** — ranged attrition. Life-draining Eldritch Blast, a Hex, Hellish Rebuke, and Dark One's Blessing.

...
export function HostSetup() {
  ...
  const onCreate = async (): Promise<void> => {
    ...
    setCreating(true);
    try {
      // The single-fight test loadout is now chosen in the lobby (re-openable between
      // attempts), so room creation just carries the boss + mode selection.
      await hostGame();
    } catch (err) {
  ...
  return (
    <div className="wb-screen">
      <div className="wb-panel wb-setup-panel" ref={panelRef}>
        ...
        {/* The single-fight test loadout (subclass skills, boons, ranks, grands and
            multiclass) is now chosen in the LOBBY, where it can be re-opened and adjusted
